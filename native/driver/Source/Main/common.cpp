    ...
    public CUnknown    
{
    private:
        ...
        PCMixBridgeHW   m_pHW;                  // Virtual Simple Audio Sample HW object
    ...
    public:
        ...
        ( 
            ...
            _In_reads_opt_(cPropertyCount) const MIXBRIDGE_DEVPROPERTY   * pProperties,
...
NTSTATUS MixBridgeDriverIoSetDeviceInterfacePropertyDataMultiple
(
    _In_ PUNICODE_STRING                                        SymbolicLinkName,
    _In_ ULONG                                                  cPropertyCount,
    _In_reads_opt_(cPropertyCount) const MIXBRIDGE_DEVPROPERTY        *pProperties
...
--*/
{
    ...
    // Initialize HW.
    // 
    m_pHW = new (POOL_FLAG_NON_PAGED, MIXBRIDGE_POOLTAG)  CMixBridgeHW;
    if (!m_pHW)
    {
...
--*/
{
    ...
    //
    ntStatus = MixBridgeDriverIoSetDeviceInterfacePropertyDataMultiple(AudioSymbolicLinkName, cPropertyCount, pProperties);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: MixBridgeDriverIoSetDeviceInterfacePropertyDataMultiple(...): failed, 0x%x", ntStatus)),
        Done);
...
)
{
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology || NULL == unknownMiniTopo)
    {
        ...
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->TopoName, // make sure this name matches with MIXBRIDGE.<TopoName>.szPname in the inf's [Strings] section
    ...
    if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave)
    {
        ...
        //
        ntStatus = InstallSubdevice(Irp,
                                    MiniportPair->WaveName, // make sure this name matches with MIXBRIDGE.<WaveName>.szPname in the inf's [Strings] section

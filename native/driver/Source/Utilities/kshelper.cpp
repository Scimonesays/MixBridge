...
#pragma code_seg("PAGE")
NTSTATUS
MixBridgePropertyDispatch
(
    _In_ PPCPROPERTY_REQUEST PropertyRequest
)
/*++
    Handles and dispatches a MIXBRIDGE_PROPERTY_ITEM.

    Use this as the property handler only if the property item is a
    MIXBRIDGE_PROPERTY_ITEM.
--*/
{
    PAGED_CODE();

    MIXBRIDGE_PROPERTY_ITEM* item = (MIXBRIDGE_PROPERTY_ITEM*)PropertyRequest->PropertyItem;

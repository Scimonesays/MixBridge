  ...
  while (true) {
    ...
    if (cmd == "PING") {
      ...
      std::snprintf(buf, sizeof(buf),
                    "OK STATE %s FRAMES %llu XRUNS %llu UNDERRUNS %llu OVERRUNS %llu LAT_MS %.2f",
                    ...
                    static_cast<unsigned long long>(d.underruns),
                    static_cast<unsigned long long>(d.overruns),
      ...
      for (const auto& d : devices) write_line(pipe, "DEVICE " + wide_to_utf8(d.name));
